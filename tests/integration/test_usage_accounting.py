...
async def test_usage_summary_today_mtd_projection_and_warnings(admin_engine) -> None:
    ...
    try:
        ...
        # SqlUsageRecorder (finding #2: "usage accounting uses a safe
        # independent transaction path") now takes the sessionmaker
        # itself, not a bound session -- each record_* call commits its
        # own short-lived session.
        recorder = SqlUsageRecorder(ingest_sessionmaker)
