"""``convergence_events`` — MASTER_SPEC.md Phase 8 (CONVERGENCE +
NEGATIVE EVIDENCE), section 59 (CONVERGENCE SURPRISE).

One row per (token, episode) -- a token's own first wave of tracked-wallet
interest within a configured window -- carrying raw wallet count,
cluster-corrected effective independent-actor count, and a non-parametric
empirical overlap probability/surprisal built from prior episodes' own
observed independent-actor counts. ``config_hash`` is part of this
table's own unique identity (the same F5-05 pattern Phase 5 established)
so a changed window/independence policy/algorithm always produces a new
...
class ConvergenceEvent(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("raw_wallet_count >= 1", name="ck_convergence_events_raw_count_positive"),
        CheckConstraint(
        ...
        CheckConstraint("expected_overlap >= 0", name="ck_convergence_events_expected_nonneg"),
        CheckConstraint(
            "empirical_probability > 0 AND empirical_probability <= 1",
            name="ck_convergence_events_probability_range",
        ),
        CheckConstraint("surprisal >= 0", name="ck_convergence_events_surprisal_nonneg"),
        CheckConstraint("sample_size >= 0", name="ck_convergence_events_sample_size_nonneg"),
        CheckConstraint("window_end >= window_start", name="ck_convergence_events_window_order"),
        CheckConstraint(
    ...
    convergence_event_id: Mapped[uuid.UUID] = mapped_column(
    ...
    # Never a 0-100 score (section 59's own explicit prohibition until
    # calibration is defined) -- a disclosed sample-size bucket instead.
